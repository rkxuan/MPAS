...
from mpma.environment.agents import KnowledgeableExpert
...
class Evaluator():
    ...
    async def evaluate_direct_answer(self,
        ...
        io_agent = KnowledgeableExpert(dataset.get_domain(), self._model_name)

        accuracy = Accuracy()
        for i_question, record in enumerate(dataset):
            io_agent.flush_memory()
            print(f"Evaluation sample {i_question}:")
            if limit_questions is not None:
                if i_question >= limit_questions:
                    break

            raw_inputs = dataset.record_to_system_input(record)
            #print("question:", raw_inputs.task)
            task_id = shortuuid.ShortUUID().random(length=5)
            io_agent.create_memory(task_id)
            answers = await io_agent.run(task_id, raw_inputs)
            answer = dataset.postprocess_answer(answers)
    ...
    async def evaluate_system(
        ...
        print(f"Evaluating system on {dataset.__class__.__name__} split {dataset.split}")
    ...
    async def optimize_system(
        ...
        print(f"Optimizing system on {dataset.__class__.__name__} split {dataset.split}")

        optimizer = torch.optim.Adam(self._system.connection_dist.parameters(), lr=lr)

        def infinite_data_loader() -> Iterator[pd.DataFrame]:  # 这个loader是可以无限加载数据的
        ...
        for i_batch in range(num_iters):
            ...
            for i_record, record in zip(range(batch_size), loader):
                edgelist, log_prob, sparse, balance  = self._system.connection_dist.realize(
                    # temperature=3.0, # DEBUG
                )
                input_dict = dataset.record_to_system_input(record)
            ...
            for answers, log_prob, correct_answer in zip(answers_batch, log_probs, correct_answers):
                ...
                single_loss = - log_prob * utility + alpha1 * sparse + alpha2 * balance
